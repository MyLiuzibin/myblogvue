...
//公共路径
let portUrl = "http://47.112.254.112:9001/api/port/v01";
// let portUrl = "http://localhost:9001/api/port/v01";

//用户注册
const getRegister = (username, password, email, callback) => {
  let url = portUrl + '/registerUsers';
  axios.post(url, {
    'userName': username,
    'password':password,
    'email':email,
    ...
    if (num.data.resultCode == 200){
      callback && callback(num.data);
    }else{
      alert(num.data.resultMessage)
...
//查询文章列表
const ShowArticlesAll = (pageNo, callback) => {
  var url = portUrl + '/ArticleList';
  axios.post(url, {
    'pageNo': pageNo
  }).then(num => {
    // console.log(num.data)
...
const ShowBrowseCount = (callback) => {
  let url = portUrl + '/ArticleTop';
  axios.post(url, {}).then(num => {
    // console.log(num.data)
    callback && callback(num.data.list);
  })
}
...
//文章评论
const setArticleComment = (content, user_id, article_id, leave_pid, pid, name, callback) => {
  let url = portUrl + '/insertComments.do';
  axios.post(url, {
...
const getArtLikeCollect = (userId, artId, islike, callback) => {
  var url = '';
  if (islike == 1) {
    url = portUrl + '/ArtLike.do';
  } else {
    url = portUrl + '/ArtCollect.do';
  }
  axios.post(url, {
    'userId':userId,
    'articleId':artId
