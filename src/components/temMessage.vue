      ...
      <div class="tmsg-commentshow">
        <ul class="tmsg-commentlist">
          <li class="tmsg-c-item" v-for="(item,index) in commentList" :key="'common'+index">
            <article class="">
              <header>
                <img :src="'\'static/img/tou2.jpg\''"  :onerror="$store.state.errorImg" >
                <div class="i-name">
                  {{item.userName}}
                </div>
                <div class="i-class">
              ...
              <section style="word-break: break-all;width: 450px;">
                <p v-html="analyzeEmoji(item.commentContent)">{{analyzeEmoji(item.commentContent)}}</p>
  ...
  export default {
    ...
    methods: { //事件处理器
        ...
        if (localStorage.getItem('userInfo')) {
          if (that.textarea) {
            ...
            if (that.leaveId == 0) {
              ...
              setArticleComment(that.textarea, that.userId, that.aid, that.pid, that.name, function (msg) {
                //   console.log(msg);
                that.textarea = '';
                that.routeChange();
                that.removeRespond();
      ...
      showCommentList: function (initData) {//评论列表
        var that = this;
        that.aid = that.$route.params.aid == undefined ? 1 : that.$route.params.aid;//获取传参的aid
        //判断当前用户是否登录
        if (localStorage.getItem('userInfo')) {
          that.haslogin = true;
          that.userInfo = JSON.parse(localStorage.getItem('userInfo'));
        ...
        //是否重新加载数据 还是累计加载
        that.pageId = initData ? 1 : that.pageId;

        //公用设置数据方法
        function setData(result) {
          // console.log("留言数据",result.list);
          if (result.resultCode == 200) {//查询数据
            var msg = result.list;
            if (result.totalCount < that.pageId * 10) {
              that.hasMore = false
            } else {
              that.hasMore = true;
            }
            that.commentList = initData ? msg : that.commentList.concat(msg);
            that.pageId = result.nextPage;
          } else {//查询数据为空
            that.hasMore = false;
