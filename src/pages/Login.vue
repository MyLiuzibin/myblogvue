...
<template>
    <div>
        <div class="container">
            <h1 class="loginTitle">
                <a href="#/">{{this.$store.state.themeObj.user_start!=0?"ZiBin 的博客":"Bin 的博客"}}</a>
            </h1>
            <!-- 登录注册 -->
                ...
                if(!that.emailErr&&!that.passwordErr){
                    UserLogin(that.email,that.password,function(msg){
                        ...
                        if(msg.resultCode==200){//登录成功
                            ...
                            that.loginErr = true;
                            that.loginTitle = msg.resultMessage;
                        }else {
                          that.loginErr = true;
